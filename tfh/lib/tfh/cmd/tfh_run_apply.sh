...
tfh_run_apply () {
  run_ids="$1"
  comment="${2:-"Confirmed-by-tf-helper"}"
  ...
  if [ -z "$run_ids" ]; then
    # get the run ID of the last run in an confirmable state and use that
  ...
  for run_id in $run_ids; do
    ...
    if ! apply_resp="$(tfh_api_call -d "$payload" "$url")"; then
      echoerr "unable to confirm run $run_id"
      return 1
    fi

    echo "confirmed run $run_id"
  done
}
